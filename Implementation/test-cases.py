...
has_output = df[df["error expected?"] == False]
print(f"Tests with output: {len(has_output.index)}")
is_csv = has_output[has_output["data format"] == "CSV"]
print(f"CSV tests with output: {len(is_csv.index)}")
